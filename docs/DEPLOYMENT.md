...
Clawd 动画继续使用 7 天浏览器缓存，以减少重复访问时的下载量。构建器会根据每组 GIF 与静态备用图的实际内容生成版本参数，例如
`clawd-notification.gif?v=12位内容指纹`。因此可以直接替换同名素材：内容变化时地址会自动变化，浏览器和 Cloudflare 不会继续复用旧图片；
内容未变化时仍能命中长期缓存。不要手工删除版本参数或为了更新图片而关闭缓存。

Clawd 以第一次接入时的动画比例为基础：`typing` 为 `115 × 157`、`thinking` 为 `127 × 142`，
`idle` 为改善视觉一致性补足为 `127 × 142`；写作状态的 `notification` 按完整通知动作使用 `147 × 157` 画布。替换素材时应保留角色原始像素，
通过透明留白适配对应槽位，并让动画内容靠右、靠底对齐，不要直接拉伸角色。
