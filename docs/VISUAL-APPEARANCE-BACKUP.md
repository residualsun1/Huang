...
| 外观对象 | 当前参数 | 视觉作用 |
| --- | --- | --- |
| 卡片底色 | `--project-card-surface: #eee8de` | 比页面 `#f2ede3` 略深，建立层次 |
| 卡片纹理 | `var(--paper-surface)` | 与全站相同的颗粒和斜纹 |
| 默认边框 | `--project-card-border: rgba(71, 65, 58, 0.18)` | 暖灰褐纸边 |
| Hover 边框 | `rgba(71, 65, 58, 0.3)` | 增强可点击感 |
| 键盘聚焦边框 | `rgba(139, 69, 19, 0.34)` | 棕色焦点提示 |
| 标题 | `#1d1b1b` | 主要信息 |
| 简介 | `#34312f` | 比纯黑柔和 |
| 箭头默认 | `#8a8680` | 弱化 |
| 箭头 Hover | `#1d1b1b` | 强化 |
| 仓库/网址入口 | `rgb(79, 77, 74)` | 与社交入口一致 |
| 入口下划线 | `rgb(197, 193, 187)` | 与社交入口一致 |
| 入口 Hover | `rgb(29, 27, 27)` | 图标、文字和线统一变深 |
| 未配置链接 | `opacity: 0.48` | 保留位置但表示不可点击 |
...
第一层偏移阴影模拟纸片厚边，第二层模糊阴影让卡片从页面上轻微抬起。
