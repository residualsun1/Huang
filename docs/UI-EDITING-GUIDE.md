...
5. 修改项目卡片外观：在 `public/styles.css` 搜索 `.project-grid`、`.project-card`、`.project-card::before`、`.project-card::after`、`.status-dot` 和 `.project-card-actions`；卡片底色继承 `--background-100`，纸纤维强度主要由 `::before` 的 `contrast` 与 `opacity` 控制，磨砂感主要由 `::after` 的渐变和 `opacity` 控制；纸纹文件位于 `public/images/project-card-paper.webp`。
6. 修改项目卡片结构或字段：在 `scripts/build.mjs` 搜索 `projectStatus`、`projectAction` 和 `const projects`。
7. 设置项目状态与链接：打开对应的 `content/projects/年份/项目名.md`，填写 `status`、`repository` 和 `website`。
