...
5. 修改项目卡片外观：在 `public/styles.css` 搜索 `.project-grid`、`.project-card`、`.project-card-actions` 和手机端的 `.home .project-grid`；卡片底色与纸张纹理由 `--project-card-surface` 和 `--paper-surface` 共同控制。
